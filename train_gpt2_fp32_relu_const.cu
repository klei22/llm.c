...
__global__ void relu_forward_kernel(float* out, const float* inp, float constant, int N, int T) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N * T) {
        int pos = idx % T;
        float value = inp[idx];
        value = value > 0 ? value : 0; // ReLU
        out[idx] = value / constant;
    }
}
...
__global__ void relu_backward_kernel(float* dpreatt, const float* datt, const float* preatt, float constant, int B, int T, int C) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < B * T * C) {
        float grad = datt[idx];
        float preatt_val = preatt[idx];
        dpreatt[idx] = (preatt_val > 0 ? grad / constant : 0); // ReLU backward
    }
}
...
void attention_forward(float* out, float* qkvr, float* att,
    ...
    float constant = 1.0f; // Replace with desired constant
    ...
    relu_forward_kernel<<<grid_size, relu_block_size>>>(att, preatt, constant, B * NH, T);
    cudaCheck(cudaGetLastError());
...
void attention_backward(float* dinp, float* dqkvr, float* dpreatt, float* datt, float* scratch,
    ...
    float constant = 1.0f; // Replace with the same constant used in the forward pass
    ...
    relu_backward_kernel<<<dim3(T / 4, B * NH), 256>>>(dpreatt, datt, att, constant, B, T, C);
    cudaCheck(cudaGetLastError());
